...
def full_test(boundary: Literal["PEC", "Periodic"], solution: int, iters: int, sizes: Tuple[int], simulation_type: int, npy: bool, error: float = 0, caps: Union[list[list[int]], Literal['d']] = 'd', zero_thres: float = 0, tsubstep: int = 1, m: int = 10, a: Tuple[float, float] = (-0.488, 0.145), p: Tuple[int, int] = (5, 20), device: Literal['cpu', 'cuda'] = 'cuda', save_type: int = 0, eps: float = 8.854e-12, mu: float = np.pi * 4e-7, ignore_error: bool = False):
    ...
    if not ignore_error:
        if not analytic: #Approximate non-analytic solutions using pre-solved solution using interpolation
            ...
            for d in order:
                EH['solver'][d] =lambda x,y,z,t,g: solved(x,y,z,t,d,g)
    ...
    while(grid_size <= end_size and (analytic or grid_size <= solver_size)):
        ...
        if analytic:
            if npy:
                for d in order:
                    EH['basic'][d] = EH['solver'][d](X[ixer[d]], Y[ixer[d]], Z[ixer[d]], yee[d][3] * t, grid_size)
            else:
                for d in order:
                    EH['basic'][d] = torch.tensor(EH['solver'][d](X[ixer[d]].cpu().numpy(), Y[ixer[d]].cpu().numpy(), Z[ixer[d]].cpu().numpy(), yee[d][3] * t, grid_size), device = device, dtype = precision)
        elif boundary == "PEC":
            if solution == 2:
                EH['basic']['Ex'] = torch.zeros((grid_size - 1, grid_size, grid_size), device = device, dtype = precision)
                EH['basic']['Hy'] = sum([np.sqrt(eps/mu) * 2 * torch.sin(np.pi * i * (X[1::2, ::2, 1::2] + c * t / 2)) * torch.sin(np.pi * i * (Y[1::2, ::2, 1::2] + c * t / 2)) * torch.sin(np.pi * i * (Z[1::2, ::2, 1::2] + c * t / 2)) for i in range(1, n + 1)])
        ...
        if not ignore_error:
            if not analytic:
                ...
                for d in order:
                    EH['solved'][d] = EH['solver'][d](*np.meshgrid(*(gu[ixer[d][i]] for i in range(3)), indexing = 'ij'), (iters * grid_size * tsubstep + yee[d][3]) * t, grid_size)
        if simulation_type == 0:
